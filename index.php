<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if ($cocok > 0){
            if ($r['level']=='Warga') {
                echo "<script>window.alert('Anda tidak dapat masuk!');</script>";
            }else{
                session_start();
                ...
                if ($r[status]=='Aktif') {       
                    ...
                    echo "<script>window.alert('Akun anda telah dinonaktifkan!');</script>";
...
<body class="login-page">
    <div class="login-box">
        <div class="logo">
            ...
            ?>
            <a href="javascript:void(0);"><img src="images/pemkot2.png" style="width: 13%; height: 10%;"/>&nbsp;&nbsp;<b><?php echo $row['nama_kel']; ?></b></a>
            
                <?php
    $query="SELECT  nama_aplikasi
            FROM desain WHERE pengguna='Admin'";
    $hasil = mysqli_query($conn, $query);  
    $jum=mysqli_num_rows($hasil);
    while($baris = mysqli_fetch_array($hasil)) {
        echo "<small>".$baris['nama_aplikasi']."</small>";
    }
    ?>
        </div>
        <div class="card">
